...
#Create Tables from Prep
x1 <- c(2, 2.5, 2.9, 2.99, 2.999, 3, 3.001,3.01, 3.1, 3.5)
x2 <- c(1, 0.5, 0.01, 0.001, 0, -0.001, -0.01, -0.5, -1)
x3 <- c(1, 1.5, 1.99, 1.999, 2, 2.001, 2.01, 2.5, 3)
x4 <- c(2.5, 2.7, 2.74, 2.749, 2.75, 2.751, 2.76, 2.8, 3)

slope1 <- (f1(x1)-f1(3))/(x1-3)
data.frame(x = x1, y = f1(x1), slope = slope1)
slope2 <- (f2(x2)-f2(0))/(x2-0)
data.frame(x = x2, y = f2(x2), slope = slope2)
slope3 <- (f3(x3)-f3(2))/(x3-2)
data.frame(x = x3, y = f3(x3), slope = slope3)
slope4 <- (f4(x4)-f4(2.75))/(x4-2.75)
data.frame(x = x4, y = f4(x4), slope = slope4)
